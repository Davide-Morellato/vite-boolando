      ...
      <ul>
        ...
        <li v-for="(discount, i) in item.badges" :key="i">
          <div v-if="discount.value === '-50%' && discount.type === 'discount'">
            <span class="price"
              >{{ (item.price - item.price * 0.5).toFixed(2) }} €</span
            >
            <span class="price-cout">{{ item.price }} €</span>
          </div>
          <div
            v-else-if="
              discount.value === '-30%' && discount.type === 'discount'
            "
          >
            <span class="price"
              >{{ (item.price - item.price * 0.3).toFixed(2) }} €</span
            >
            <span class="price-cout">{{ item.price }} €</span>
          </div>
          <div
            class="price"
            v-else-if="item.badges.length < 2 && discount.type === 'tag'"
          >
